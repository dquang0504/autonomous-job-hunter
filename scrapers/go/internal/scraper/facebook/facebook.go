...
func (s *FacebookScraper) Scrape(ctx context.Context, browserCtx playwright.BrowserContext) ([]scraper.Job, error) {
	...
	for _, groupUrl := range groups {
		...
		for i := 0; i < maxPostsToCheck; i++ {
			...
			jobTime := extractFacebookPostTimestamp(page, post)
			...
			res, err := classifier.ClassifyWithFastText(ctx, bodyText)
			if err != nil {
				log.Printf("      ⚠️ FastText Python classifier error: %v. Falling back to Naive Bayes.", err)
				if seedModel != nil {
					res = classifier.ClassifyWithSeedModel(seedModel, bodyText)
				} else {
					res = classifier.ClassificationResult{IsHiring: true, Confidence: 0.5} // default permissive on fallback fail
				}
			}
...
// extractFacebookPostTimestamp extracts the post timestamp by performing highly specialized pixel coordinate hovering and text selection (dragging)
func extractFacebookPostTimestamp(page playwright.Page, post playwright.Locator) string {
	jobTime := "Recent"
	...
	for _, svg := range svgs {
		...
		if err == nil && box != nil && box.Width <= 20 && box.Height <= 20 {
			targetSvg = svg
			break
		}
	}
